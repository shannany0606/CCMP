...
def eval_mask(gt_masks: np.ndarray, fake_masks: np.ndarray):
    ...
    """
    iou = eval_metrics.db_eval_iou(gt_masks, fake_masks)
    boundary = eval_metrics.db_eval_boundary(gt_masks, fake_masks)
    return iou, boundary
...
def location_score(gt_mask, pred_mask, size=(480, 480)):
    H, W = size
    (gt_size, pred_size), (centroid_gt, centroid_pred), (gt_compact_mask, pred_compact_mask) = eval_metrics.crop_mask(gt_mask, pred_mask)
